...
# Load data
x_train = torch.load(os.path.join(torch_tensors_path, 'x_train_pet.pt'))
x_test = torch.load(os.path.join(torch_tensors_path, 'x_test_pet.pt'))
...
def objective(trial):
    ...
    model = PET_model(x_train.shape, len(np.unique(y_train)), dropout=dropout_rate).cuda()
    ...
    for epoch in range(50):
        ...
        for images, labels in train_loader:
            train, labels = images.cuda(), labels.cuda()
            optimizer.zero_grad()
            outputs = model(train)
        ...
        with torch.no_grad():
            for images, labels in val_loader:
                val, labels = images.cuda(), labels.cuda()
                outputs = model(val).detach()
...
plt.title('Accuracy Metrics PET Model', fontsize=28)
...
plt.title('Loss Metrics PET Model', fontsize=28)
plt.xticks(fontsize=24)
plt.yticks(fontsize=24)
plt.legend(fontsize=24)

plt.savefig('graphs/training_metrics_PET.jpg')
plt.close()
...
if isinstance(model, nn.DataParallel):
    torch.save(model.module.state_dict(), os.path.join(trained_model_path, 'pet_model.pt'))
else:
    torch.save(model.state_dict(), os.path.join(trained_model_path, 'pet_model.pt'))

model.eval()
with torch.no_grad():
    for images, labels in test_loader:
        test, labels = images.cuda(), labels.cuda()
...
print('')
print_metrics(all_labels, all_predictions, conf_matrix=True, classes=['AD', 'CN', 'MCI'], filename='PET_training_confusion_matrix')
